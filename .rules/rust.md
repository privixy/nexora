...
1. **Repository Structure:** Follow `docs/architecture/repository-structure.md` for current Rust source/test ownership. Root `src-tauri/` remains the enforced Tauri crate path until the desktop migration lands.
2. **Keep `mod.rs` focused:** In `src-tauri/src/**/mod.rs`, keep public orchestration, exports, and trait implementations. Extract pure helpers, parsers, and driver-specific utilities into dedicated sibling modules.
