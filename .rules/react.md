# React Rules
1. **Repository Structure:** Follow `docs/architecture/repository-structure.md` for current React source/test ownership. Desktop source, assets, tests, dependencies, scripts, and app-local frontend configuration live under `apps/desktop/` and must not be reintroduced at root; root `tests/repository/` contains repository contracts only.
