...
describe('isMultiDatabaseCapable', () => {
  ...
  it('returns false when schemas is true without an explicit flag', () => {
    expect(isMultiDatabaseCapable({ ...baseCapabilities, schemas: true })).toBe(false);
  });

  it('returns true when multiple_databases is explicitly enabled', () => {
    expect(isMultiDatabaseCapable({
      ...baseCapabilities,
      schemas: true,
  ...
  it('returns false when multiple_databases is explicitly disabled', () => {
    expect(isMultiDatabaseCapable({
      ...baseCapabilities,
  ...
  it('honors the explicit flag for no-connection drivers', () => {
    expect(isMultiDatabaseCapable({
      ...baseCapabilities,
