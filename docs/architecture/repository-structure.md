...
- Governed production source roots accept JavaScript-family modules only as `.ts` or `.tsx`; `.js`, `.jsx`, `.mjs`, `.cjs`, `.mts`, and `.cts` are rejected so architecture controls inventory every executable frontend module.
...
- a peer `*_tests.rs`, non-trivial inline Rust test module under any nested `cfg` predicate containing `test`, test-source `include!`, production test-gated `include!`, any production/test `#[path]` or `cfg_attr(..., path = ...)` inclusion, or unclassified conventional, nested, or custom Cargo integration target is added;
...
File-size baselines must equal current line counts for files still above the soft limit; stale values fail and baselines must be shrunk automatically during the change, while entries that fall below the limit must be removed. Former oversized compatibility files `commands.rs`, `config.rs`, `health_check.rs`, `mcp/mod.rs`, `plugins/driver.rs`, and `pool_manager.rs` have no stale baseline; retained oversized implementation/test owners are explicitly ratcheted. Rust inline-test debt is closed with empty desktop and template inline-test allowlists; detection covers `cfg(all(test))` and whitespace variants. Production modules and Rust test sources reject every `#[path]`/`cfg_attr(..., path = ...)` inclusion, and test sources reject all `include!` delimiter variants. `rustIntegrationTests` records every conventional, nested, and custom Cargo integration target with the supported classification enum, matching default mode, and exact explicit Cargo command. `database_integration.rs` is the single canonical external-infrastructure suite with ten ignored tests. Cargo test inventory commands run through `mise exec -- cargo`.
