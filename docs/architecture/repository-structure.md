...
- Rust unit tests use sibling `tests.rs`; crate integration tests use `apps/desktop/src-tauri/tests/`. Thirty-four suites are module-local under their production owners; two crate-level peer suites remain as temporary compatibility exceptions.
...
| 2 crate-level Rust `*_tests.rs` peer suites | Backend maintainers | Existing convention debt after 34 suites moved to module-local ownership | Test normalization |
