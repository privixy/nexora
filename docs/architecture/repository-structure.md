...
- Rust unit tests use sibling `tests.rs`; crate integration tests use `apps/desktop/src-tauri/tests/`. Twenty-six suites are module-local under their production owners; three crate-level peer suites remain as temporary compatibility exceptions.
...
| 3 crate-level Rust `*_tests.rs` peer suites | Backend maintainers | Existing convention debt after 26 suites moved to module-local ownership | Test normalization |
