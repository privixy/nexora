...
- a peer `*_tests.rs`, non-trivial inline Rust test module, test-source `include!`, any production/test `#[path]` or `cfg_attr(..., path = ...)` inclusion, or unclassified conventional, nested, or custom Cargo integration target is added;
