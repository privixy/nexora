...
const PROJECT_ORGANIZATION: &str = "nexora";
const PROJECT_APPLICATION: &str = "Nexora";

pub(crate) fn plugin_project_dirs() -> Option<ProjectDirs> {
    ProjectDirs::from(PROJECT_QUALIFIER, PROJECT_ORGANIZATION, PROJECT_APPLICATION)
}

pub fn get_plugins_dir() -> Result<PathBuf, String> {
    let proj_dirs = plugin_project_dirs()
