...
fn get_all_clients() -> Vec<McpClient> {
    ...
    let opencode_path = base
        .as_ref()
        .map(|b| b.config_dir().join("opencode/opencode.json"));
...
fn read_json_config(path: &PathBuf) -> Result<serde_json::Value, String> {
    if !path.exists() {
        return Ok(json!({}));
    }
...
fn is_nexora_in_mcp_servers(path: &PathBuf) -> bool {
    read_json_config(path)
        .ok()
        .and_then(|j| j.get("mcpServers").cloned())
        .map(|s| s.get("nexora").is_some())
        .unwrap_or(false)
}

/// Command-driven clients store MCP config in different formats; for status
/// purposes, consider Nexora installed when their config references it.
fn is_command_client_installed(path: &PathBuf) -> bool {
    ...
    fs::read_to_string(path)
        .map(|c| {
            c.contains("\"nexora\"")
                || c.contains("[mcp_servers.nexora]")
                || c.contains("[mcp_servers.\"nexora\"]")
        })
        .unwrap_or(false)
}

fn is_opencode_client_installed(path: &PathBuf) -> bool {
    read_json_config(path)
        .ok()
        .and_then(|j| j.get("mcp").cloned())
        .map(|m| m.get("nexora").is_some())
        .unwrap_or(false)
}
...
fn write_mcp_config(client: &McpClient, exe_str: &str) -> Result<(), String> {
    if client.client_type == "manual" {
        return Err("This client uses manual configuration only".to_string());
    }

    let config_path = client
        .config_path
        .as_ref()
        .ok_or_else(|| "Could not determine config path for this OS".to_string())?;
    ...
    if client.client_type == "opencode" {
        if config.get("mcp").is_none() {
            config["mcp"] = json!({});
        }
        config["mcp"]["nexora"] = json!({
            "type": "local",
            "command": [exe_str, "--mcp"],
            "enabled": true
        });
    ...
    let installed = match client.client_type {
        "opencode" => is_opencode_client_installed(config_path),
        _ => is_nexora_in_mcp_servers(config_path),
    };

    if !installed {
        return Err(format!(
            "Wrote config but could not verify Nexora in {}",
            config_path.display()
        ));
    }
...
pub async fn install_mcp_config<R: Runtime>(
    ...
    write_mcp_config(client, &exe_str)?;
