...
#[derive(Debug, Serialize, Deserialize, Clone)]
pub struct NexoraSelfStats {
    pub pid: u32,
    /// RSS of the main nexora process only (bytes).
    ...
    // children are fetched on-demand via get_nexora_children, not on every poll
}
...
pub struct SystemStats {
    ...
    pub nexora: Option<NexoraSelfStats>,
}
...
fn refresh_and_collect_system_stats() -> SystemStats {
    ...
    // Nexora self stats.
    let nexora = get_current_pid().ok().map(|self_pid| {
...
fn collect_nexora_children() -> Vec<NexoraChildProcess> {
    let mut sys = SYSTEM.lock().expect("system mutex poisoned");
    sys.refresh_processes_specifics(
        ProcessesToUpdate::All,
        true,
    ...
    let mut children: Vec<NexoraChildProcess> = descendants
        .iter()
        .filter_map(|pid| {
            sys.process(*pid).map(|proc| NexoraChildProcess {
                pid: pid.as_u32(),
...
#[tauri::command]
pub async fn get_nexora_children() -> Result<Vec<NexoraChildProcess>, String> {
    tokio::task::spawn_blocking(collect_nexora_children)
        .await
        .map_err(|e| format!("Failed to collect nexora children: {}", e))
}
...
pub async fn open_task_manager_window(app: AppHandle) -> Result<(), String> {
    ...
    .title(format_window_title(Some("Task Manager")))
