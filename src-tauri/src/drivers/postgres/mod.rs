...
async fn reset_pg_search_path(client: &deadpool_postgres::Client) -> Result<(), String> {
    client
...
fn update_record_error_context(
    ...
    let hint = if matches!(new_val, serde_json::Value::String(_))
        ...
        "\nHint: the edited value arrived from the grid as a JSON string. For numeric PostgreSQL columns, Nexora attempts to parse it before binding; check that the value is valid for the target column type."
    } else {
        ""
    };
...
pub async fn execute_query(
    ...
    let result = exec_on_pg_client(&client, query, limit, page).await;
    let reset_result = if schema.is_some() {
        reset_pg_search_path(&client).await
    } else {
        Ok(())
    };
    drop(client);
...
impl DatabaseDriver for PostgresDriver {
    ...
    async fn create_database(
        ...
        execute_query(&p, &sql, None, 1, None).await.map(|_| ())
    }
    ...
    async fn drop_database(
        ...
        let mut p = params.clone();
        let maintenance_database = if database == "postgres" {
            "template1"
        } else {
            "postgres"
        };
        p.database = crate::models::DatabaseSelection::Single(maintenance_database.to_string());
        let escaped_database = escape_identifier(database);
        let escaped_literal = database.replace('\'', "''");
        let terminate_sql = format!(
            "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '{escaped_literal}' AND pid <> pg_backend_pid()",
        );
        execute_query(&p, &terminate_sql, None, 1, None).await?;
        let sql = format!("DROP DATABASE \"{}\"", escaped_database);
        execute_query(&p, &sql, None, 1, None).await.map(|_| ())
    }
    ...
    async fn create_schema(
        ...
        let sql = format!("CREATE SCHEMA \"{}\"", escape_identifier(schema));
    ...
    async fn truncate_table(
        ...
        execute_query(params, &sql, None, 1, Some(schema))
            .await
            .map(|_| ())
    }
