...
//! Nexora executable, ssh re-runs this binary with the prompt as its only
//! argument. This module detects that situation (via the endpoint env var the
//! server injected), forwards the prompt to the main Nexora process over a
...
pub const SOCKET_ENV: &str = "NEXORA_ASKPASS_SOCKET";
/// Windows only: shared secret proving the client was spawned by Nexora.
#[cfg(windows)]
pub const TOKEN_ENV: &str = "NEXORA_ASKPASS_TOKEN";
...
fn run(endpoint: &str, kind: PromptKind, prompt: &str) -> i32 {
    let stream = match connect(endpoint) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("[Askpass] Failed to reach Nexora: {}", e);
            return 1;
        }
    };
...
fn exchange<S: Read + Write>(
    ...
    decode_response(line.trim_end_matches(['\n', '\r']))
        .ok_or_else(|| "Malformed response from Nexora".to_string())
}
