...
export const EditorProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const loadPreferences = async () => {
      setIsLoading(true);
      try {
        const { tabs: loadedTabs } = await loadEditorPreferences(activeConnectionId);
        ...
        if (loadedTabs.length > 0) {
          ...
          setTabs((prev) => {
            ...
            return [...tabsFromOtherConnections, initialTab, ...loadedTabs];
          });
  ...
  const getSchema = useCallback(
    async (
      ...
      const cacheKey = `${connectionId}:database=${database ?? ""}:schema=${schema ?? ""}`;
