...
import { generateVisualQuerySQL, type WhereCondition, type OrderByClause } from './visualQuery';
...
const VisualQueryBuilderContent = () => {
  ...
  const createTableNode = useCallback(
    (tableName: string, columns: TableColumn[], database?: string, schema?: string, position = { x: 80, y: 80 }): Node => {
    ...
    [deleteNode, onColumnAlias, onColumnAggregation, onColumnCheck],
  );
  ...
  useEffect(() => {
    const tableName = activeTab?.activeTable;
    if (!tableName || !activeConnectionId || nodes.length > 0) return;

    let isMounted = true;
    const database = activeTab?.database ?? activeDatabase ?? undefined;
    const schema = activeTab?.schema ?? activeSchema ?? undefined;
        ...
        if (isMounted) {
          setNodes((currentNodes) => (
            currentNodes.length > 0
              ? currentNodes
              : currentNodes.concat(createTableNode(tableName, columns, database, schema))
          ));
        }
      } catch (e) {
  ...
  }, [activeConnectionId, activeDatabase, activeSchema, activeTab?.activeTable, activeTab?.database, activeTab?.schema, createTableNode, nodes.length, setNodes]);
  ...
  const onPointerUp = useCallback(
    async (event: React.PointerEvent) => {
      ...
      const database = dragState.database ?? activeTab?.database ?? activeDatabase ?? undefined;
      const schema = dragState.schema ?? activeTab?.schema ?? activeSchema ?? undefined;
      ...
      try {
        ...
        setNodes((nds) => nds.concat(createTableNode(tableName, columns, database, schema, position)));
      } catch (e) {
        console.error("Failed to fetch columns", e);
      }
    },
    [activeConnectionId, activeDatabase, activeSchema, activeTab?.database, activeTab?.schema, createTableNode, screenToFlowPosition, setNodes],
  );

  const availableTableTargets: TableTarget[] = activeCapabilities?.multiple_databases === true
    ? selectedDatabases.flatMap((database): TableTarget[] => {
      const databaseData = databaseDataMap[database];
      if (activeCapabilities.schemas === true) {
        return Object.entries(databaseData?.schemaDataMap ?? {}).flatMap(([schema, schemaData]) =>
          schemaData.tables.map((table): TableTarget => ({ table: table.name, database, schema })),
        );
      }
      return (databaseData?.tables ?? (database === activeDatabase ? tables : [])).map((table): TableTarget => ({
        table: table.name,
        database,
        schema: undefined,
      }));
    ...
    : activeCapabilities?.schemas === true
      ? Object.entries(schemaDataMap).flatMap(([schema, schemaData]) =>
  ...
  const addTableToQuery = useCallback(
      ...
      try {
        ...
        setNodes((currentNodes) => currentNodes.concat(
          createTableNode(
            ...
            target.database,
            target.schema,
            { x: 80 + currentNodes.length * 40, y: 80 + currentNodes.length * 40 },
          ),
        ));
      } catch (e) {
        console.error("Failed to fetch columns", e);
    ...
    [activeConnectionId, createTableNode, setNodes],
  );
      ...
      {showSettings && (
        <div className="w-96 bg-elevated border-l border-default flex flex-col overflow-hidden shadow-2xl">
          ...
          <div className="p-5 border-b border-default">
            <div className="text-sm font-semibold text-primary mb-4">Tables</div>
            {availableTableTargets.length === 0 ? (
              <div className="text-xs text-muted italic py-2">No tables available</div>
            ) : (
              <div className="flex flex-col gap-2">
                <select
                  ...
                  defaultValue=""
                  onChange={(e) => {
                    const selected = availableTableTargets[Number(e.target.value)];
                    if (selected) {
                      addTableToQuery(selected);
                      e.target.value = '';
                    }
                  }}
                >
                  <option value="">Add table</option>
                  {availableTableTargets.map((target, index) => (
                    <option key={`${target.database ?? ''}.${target.schema ?? ''}.${target.table}.${index}`} value={index}>
                      {[target.database, target.schema, target.table].filter(Boolean).join('.')}
                    </option>
                  ))}
                </select>
                <div className="text-xs text-muted">
                  Select a table, then tick columns in the node before running.
                </div>
              </div>
            )}
          </div>
