...
export function generateTableList(
  ...
  return nodes.map((node) => {
    const tableName = formatTableReference(node.data, driver);
    const alias = aliases[node.id];
    return `${tableName} ${alias}`;
  });
}

function formatTableReference(data: TableNodeData, driver?: string | null): string {
  const qualifiers = driver === 'postgres'
    ? [data.schema]
    : [data.database, data.schema];
  const parts = [...qualifiers, data.label]
    .filter((part): part is string => Boolean(part));
  return parts.map((part) => formatSqlIdentifier(part, driver)).join('.');
}
...
export function generateFromClause(
  ...
  let sql = `\nFROM\n  ${formatTableReference(firstData, driver)} ${aliases[firstNode.id]}`;
    ...
    for (let i = 0; i < edgesToProcess.length; i++) {
      ...
      if (isSourceProcessed && !isTargetProcessed) {
        ...
        if (targetNode) {
          ...
          sql += `\n${joinType} JOIN ${formatTableReference(targetData, driver)} ${targetAlias} ON ${sourceAlias}.${edge.sourceHandle} = ${targetAlias}.${edge.targetHandle}`;
        ...
        if (sourceNode) {
          ...
          const edgeData = edge.data;
          const joinType = edgeData?.joinType || 'INNER';
          sql += `\n${joinType} JOIN ${formatTableReference(sourceData, driver)} ${sourceAlias} ON ${sourceAlias}.${edge.sourceHandle} = ${targetAlias}.${edge.targetHandle}`;
  ...
  nodes.forEach((node) => {
    if (!processedNodes.has(node.id)) {
      const data = node.data;
      sql += `,\n  ${formatTableReference(data, driver)} ${aliases[node.id]}`;
    }
  });
