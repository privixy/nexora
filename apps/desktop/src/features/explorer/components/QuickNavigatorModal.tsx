...
import { newConsoleForTable } from "../../editor";
import type { RoutineInfo, TriggerInfo } from "../../connections";

interface QuickNavigatorModalProps {
  isOpen: boolean;
  onClose: () => void;
  onGenerateSql?: (tableName: string, database?: string, schema?: string) => void;
  onInspect?: (tableName: string, database?: string, schema?: string) => void;
}
...
export const QuickNavigatorModal = ({ isOpen, onClose, onGenerateSql, onInspect }: QuickNavigatorModalProps) => {
  ...
  const handleSelect = useCallback(async (item: typeof items[number]) => {
    onClose();
    const { name, type, database, schema } = item;

    if (type === "table") {
      if (isMultiDatabaseCapable(activeCapabilities)) {
        setActiveTableContext(name, database, activeCapabilities?.schemas ? schema : null);
        const quotedTable = quoteTableRef(name, activeDriver, activeCapabilities?.schemas ? schema : undefined);
        navigate("/editor", {
          state: {
            initialQuery: `SELECT * FROM ${quotedTable}`,
            tableName: name,
            database,
            schema: activeCapabilities?.schemas ? schema : undefined,
            title: database ? `${name} (${database}${schema && schema !== database ? `/${schema}` : ""})` : name,
      ...
      if (isMultiDatabaseCapable(activeCapabilities)) {
        const quotedView = quoteTableRef(name, activeDriver, activeCapabilities?.schemas ? schema : undefined);
        navigate("/editor", {
          state: {
  ...
  const handleCountRows = useCallback((item: typeof items[number]) => {
    onClose();
    const { name, database, schema } = item;
    const resolvedSchema = activeCapabilities?.schemas ? schema : undefined;
    const quotedTable = quoteTableRef(name, activeDriver, resolvedSchema);
    navigate("/editor", {
      state: {
  ...
  const handleNewConsole = useCallback((item: typeof items[number]) => {
    onClose();
    const resolvedSchema = activeCapabilities?.schemas ? item.schema : undefined;
    const spec = newConsoleForTable(item.name, activeDriver, resolvedSchema);
    navigate("/editor", {
      state: {
                              ...
                              {onInspect && (
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    onClose();
                                    onInspect(item.name, item.database, item.schema);
                                  }}
                              ...
                              {onGenerateSql && (
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    onClose();
                                    onGenerateSql(item.name, item.database, item.schema);
                                  }}
