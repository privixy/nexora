...
const SidebarTableItemImpl = ({
  ...
  }, [connectionId, table.name, database, schema]);
  ...
  const handleContextMenu = (e: React.MouseEvent, type: string, name: string) => {
    e.preventDefault();
    e.stopPropagation();
    onContextMenu(e, type, name, name, getSidebarTableContext(table.name, database, schema));
  };
  ...
  return (
    <div className="flex flex-col">
      <div
        ...
        data-schema={schema ?? ''}
        onPointerDown={(e) => {
          dragState.start(table.name, { database, schema });
                  ...
                  <div className="ml-4 border-l border-default/50">
                    {columns.map((col) => (
                      <SidebarColumnItem
                        ...
                        onEdit={(c) => onEditColumn(table.name, c, database, schema)}
                        database={database}
                        schema={schema}
                      />
                    ))}
