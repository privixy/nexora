...
import {
  ...
  addCellToCells,
  removeCellFromCells,
} from "../../notebooks/lib/notebook";
import {
  computeAutoScrollSpeed,
  createCellDragPreview,
  moveCell,
  reorderCells,
} from "../../notebooks/lib/notebookDnd";
import {
  serializeNotebook,
  deserializeNotebook,
...
import {
  ...
  addFailure,
  addSkipped,
} from "../../notebooks/lib/notebookRunAll";
import { resolveQueryVariables, findUnresolvedDependencies } from "../../notebooks/lib/notebookVariables";
import { resolveParams } from "../../notebooks/lib/notebookParams";
import {
  addHistoryEntry,
  createHistoryEntry,
} from "../../notebooks/lib/notebookHistory";
...
} from "../../notebooks/lib/notebookUndo";
import {
  getNotebookState,
...
export function NotebookView({
  ...
  const { activeSchema, activeDatabase, activeCapabilities, selectedDatabases } = useDatabase();
  const isMultiDb =
    isMultiDatabaseCapable(activeCapabilities) && selectedDatabases.length > 1;
  const effectiveDatabase = tab.database || (isMultiDb ? activeDatabase || selectedDatabases[0] : null);
  const effectiveSchema = tab.schema || activeSchema || null;
