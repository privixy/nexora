...
describe('DatabaseProvider', () => {
  ...
  it('should update window title on connection', async () => {
    ...
    await waitFor(() => {
      expect(invoke).toHaveBeenCalledWith('set_window_title', {
        title: 'Nexora - Local MySQL (testdb)',
      });
    });
  });
  ...
  describe('PostgreSQL Schema Selection', () => {
    ...
    it('should load selected PostgreSQL database metadata without changing active database', async () => {
      const multiDatabaseConnections = [
        {
          ...mockPgConnections[0],
          params: {
            ...mockPgConnections[0].params,
            database: ['app', 'analytics'],
          },
        },
      ];
      const multiDatabaseManifest = {
        ...mockPostgresManifest,
        capabilities: {
          ...mockPostgresManifest.capabilities,
          multiple_databases: true,
        },
      };

      vi.mocked(invoke).mockImplementation((cmd: string, args?: unknown) => {
        if (cmd === 'get_connections') return Promise.resolve(multiDatabaseConnections);
        if (cmd === 'test_connection') return Promise.resolve('Connection successful!');
        if (cmd === 'get_driver_manifest') return Promise.resolve(multiDatabaseManifest);
        if (cmd === 'get_schemas') return Promise.resolve(mockSchemas);
        if (cmd === 'get_selected_schemas') return Promise.resolve(['public']);
        if (cmd === 'get_schema_preference') return Promise.resolve('public');
        if (cmd === 'get_tables') {
          const db = (args as { database?: string } | undefined)?.database;
          return Promise.resolve(db === 'analytics' ? [{ name: 'events' }] : mockPgTables);
        }
        if (cmd === 'get_views') return Promise.resolve(mockPgViews);
        if (cmd === 'get_routines') return Promise.resolve(mockPgRoutines);
      ...
      await act(async () => {
        await result.current.loadDatabaseData('analytics');
      });
    ...
    it('should activate a PostgreSQL database only when requested', async () => {
      const multiDatabaseConnections = [
        {
          ...mockPgConnections[0],
          params: {
      ...
      await act(async () => {
        await result.current.loadDatabaseData('analytics', undefined, true);
      });
    ...
    it('should route a selected PostgreSQL database separately from its schema', async () => {
      const multiDatabaseConnections = [
        {
          ...mockPgConnections[0],
          params: {
      ...
      expect(result.current.selectedDatabases).toEqual(['app', 'analytics']);
      expect(result.current.activeDatabase).toBe('app');
      expect(result.current.activeSchema).toBe('public');
      expect(invoke).toHaveBeenCalledWith('get_schemas', {
        connectionId: 'pg-conn',
        database: 'app',
      });
      ...
      expect(result.current.databaseDataMap.app?.schemas).toEqual(mockSchemas);
      expect(result.current.databaseDataMap.app?.schemaDataMap?.public?.tables).toEqual(mockPgTables);
    });

    it('should keep active database aligned with selected databases', async () => {
      const multiDatabaseConnections = [
        {
          ...mockPgConnections[0],
          params: {
      ...
      act(() => {
        result.current.setSelectedDatabases(['analytics']);
      });
    ...
    it('should prefetch all selected PostgreSQL databases after connect', async () => {
      const multiDatabaseConnections = [
        {
          ...mockPgConnections[0],
          params: {
