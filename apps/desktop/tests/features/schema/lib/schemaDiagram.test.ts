...
describe('schemaDiagram', () => {
  describe('resolveDiagramSchema', () => {
    ...
    it('should not use the selected database as schema when no schema is given', () => {
      expect(resolveDiagramSchema(undefined, 'web_opd_backend')).toBeUndefined();
    });
  ...
  describe('resolveDiagramDatabase', () => {
    it('should use the selected database for multi-database diagrams', () => {
      expect(resolveDiagramDatabase('web_opd_backend')).toBe('web_opd_backend');
    });

    it('should ignore the Unknown sentinel', () => {
      expect(resolveDiagramDatabase('Unknown')).toBeUndefined();
    });

    it('should return undefined when database is absent', () => {
      expect(resolveDiagramDatabase(undefined)).toBeUndefined();
    });
  });
  ...
  describe('parseConnectionParams', () => {
    ...
    it('should parse schema when present', () => {
      const params = new URLSearchParams({
        connectionId: 'conn-123',
        connectionName: 'Production DB',
        databaseName: 'main_db',
    ...
    it('should handle special characters in names', () => {
      ...
      params.set('connectionName', 'Production DB');
