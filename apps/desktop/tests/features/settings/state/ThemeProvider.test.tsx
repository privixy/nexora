...
import { ThemeProvider } from "../../../../src/features/settings/state/ThemeProvider";
...
describe("ThemeProvider", () => {
  ...
  it("should provide default theme when backend is empty", async () => {
    ...
    expect(result.current.currentTheme.id).toMatch(/nexora-(dark|light)/);
  ...
  it("should load theme from backend config", async () => {
    vi.mocked(invoke).mockImplementation((cmd: string) => {
      if (cmd === "get_config") {
        return Promise.resolve({ theme: "nexora-dark" });
      }
      if (cmd === "save_config") {
        return Promise.resolve(undefined);
      }
    ...
    expect(result.current.currentTheme.id).toBe("nexora-dark");
  });
    ...
    await waitFor(() => {
      expect(invoke).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          config: expect.objectContaining({ theme: "nexora-dark" }),
        }),
      );
    });

    // Check that old localStorage was removed
    expect(localStorage.getItem("nexora_theme_settings")).toBeNull();
  });
  ...
  it("should detect theme from system preferences if not set", async () => {
    ...
    vi.mocked(invoke).mockImplementation((cmd: string, args?: any) => {
      ...
      if (cmd === "save_config") {
        expect(args?.config).toHaveProperty("theme", "nexora-dark");
        return Promise.resolve(undefined);
      }
      if (cmd === "get_all_themes") {
        return Promise.resolve([]);
  ...
  it("should set theme", async () => {
    ...
    await act(async () => {
      await result.current.setTheme("nexora-dark");
    });

    expect(result.current.currentTheme.id).toBe("nexora-dark");
    expect(result.current.settings.activeThemeId).toBe("nexora-dark");
  });
  ...
  it("should not delete preset themes", async () => {
    ...
    await expect(
      result.current.deleteCustomTheme("nexora-dark"),
  ...
  it("should export theme", async () => {
    ...
    await act(async () => {
      exportedJson = await result.current.exportTheme("nexora-dark");
    });

    expect(exportedJson).toBeTruthy();
    const parsed = JSON.parse(exportedJson);
    expect(parsed.id).toBe("nexora-dark");
    expect(parsed.name).toBe("Nexora Dark");
  });
  ...
  it("should save theme to config when changed", async () => {
    ...
    vi.mocked(invoke).mockImplementation((cmd: string) => {
      if (cmd === "get_config") {
        return Promise.resolve({ theme: "nexora-light" });
      }
      if (cmd === "save_config") {
        return Promise.resolve(undefined);
      }
