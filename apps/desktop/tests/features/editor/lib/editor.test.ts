...
import {
  ...
  validatePageNumber,
  calculateTotalPages,
} from "../../../../src/features/editor/lib/editor";
...
describe("editor", () => {
  ...
  describe("findExistingTableTab", () => {
    ...
    it("should distinguish table tabs by database", () => {
      const tabs: Tab[] = [
        createMockTab({
          id: "tab-1",
          type: "table",
      ...
      const result = findExistingTableTab(tabs, "conn-1", "users", undefined, "analytics");

      expect(result?.id).toBe("tab-2");
    });

    it("should not reuse a table tab from another database", () => {
      const tabs: Tab[] = [
        createMockTab({
          id: "tab-1",
          type: "table",
