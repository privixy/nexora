...
import {
  ...
  type WhereCondition,
  type OrderByClause,
} from '../../../../src/features/editor/query-builder/visualQuery';
...
describe('visualQuery utils', () => {
  ...
  describe('generateTableList', () => {
    ...
    it('should qualify MySQL tables with their source database', () => {
      const nodes: QueryNode[] = [
        {
          id: 'n1',
          data: {
      ...
      expect(generateTableList(nodes, { n1: 't1', n2: 't2' }, 'mysql')).toEqual([
        'app.users t1',
        'analytics.events t2',
      ]);
    });

    it('should qualify PostgreSQL tables by schema without duplicating database', () => {
      const nodes: QueryNode[] = [
        {
          id: 'n1',
          data: {
      ...
      expect(generateTableList(nodes, { n1: 't1' }, 'postgres')).toEqual([
        'public.users t1',
      ]);
    });
  ...
  describe('generateFromClause', () => {
    ...
    it('should qualify joined tables with their source databases for MySQL', () => {
      const nodes: QueryNode[] = [
        {
          id: 'n1',
          data: { label: 'users', database: 'app', columns: [], selectedColumns: {} },
        },
        {
          id: 'n2',
          data: { label: 'events', database: 'analytics', columns: [], selectedColumns: {} },
        },
      ];
      const edges: QueryEdge[] = [
        {
      ...
      const result = generateFromClause(nodes, edges, { n1: 't1', n2: 't2' }, 'mysql');

      expect(result).toContain('app.users t1');
      expect(result).toContain('INNER JOIN analytics.events t2 ON t1.id = t2.user_id');
    });
  ...
  describe('generateVisualQuerySQL', () => {
    ...
    it('should generate database-qualified SQL for MySQL visual queries', () => {
      const nodes: QueryNode[] = [
        {
          id: 'n1',
          data: {
    ...
    it('should generate schema-qualified SQL for PostgreSQL visual queries', () => {
      const nodes: QueryNode[] = [
        {
          id: 'n1',
          data: {
      ...
      expect(result).toContain('FROM\n  public.users t1');
      expect(result).not.toContain('app.public.users');
    });
