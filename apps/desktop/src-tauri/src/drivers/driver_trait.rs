...
pub struct DriverCapabilities {
    ...
    /// Supports creating databases from the sidebar. Defaults to false so non-SQL plugins opt in explicitly.
    #[serde(default, alias = "createDatabase")]
    pub create_database: bool,
    /// Supports dropping databases from the sidebar. Defaults to false so non-SQL plugins opt in explicitly.
    #[serde(default, alias = "dropDatabase")]
    pub drop_database: bool,
    /// Supports renaming databases from the sidebar. Defaults to false so non-SQL plugins opt in explicitly.
    #[serde(default, alias = "renameDatabase")]
    pub rename_database: bool,
    /// Supports creating schemas from the sidebar. Defaults to false so non-SQL plugins opt in explicitly.
    #[serde(default, alias = "createSchema")]
    pub create_schema: bool,
    /// Supports truncating or clearing table-like collections from the sidebar. Defaults to false so plugins opt in explicitly.
    #[serde(default, alias = "truncateTable")]
...
pub trait DatabaseDriver: Send + Sync {
    ...
    async fn create_database(
        ...
        Err("Create database not supported by this driver".into())
    }
    ...
    async fn drop_database(
        ...
        Err("Drop database not supported by this driver".into())
    }
    ...
    async fn rename_database(
        ...
        Err("Rename database not supported by this driver".into())
    }

    async fn create_schema(&self, _params: &ConnectionParams, _schema: &str) -> Result<(), String> {
        Err("Create schema not supported by this driver".into())
    }
    ...
    async fn truncate_table(
        ...
        Err("Truncate table not supported by this driver".into())
    }
    ...
    async fn drop_table(
        ...
        Err("Drop table not supported by this driver".into())
    }
