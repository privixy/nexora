...
use crate::plugins::process::{PluginProcess, PLUGIN_INIT_TIMEOUT};
...
pub struct RpcDriver {
    ...
    pub(crate) data_types: Vec<DataTypeInfo>,
}
...
impl DatabaseDriver for RpcDriver {
    ...
    async fn create_database(
        ...
        let res = self
            ...
            .call(
                "create_database",
                json!({ "params": params, "database": database }),
            )
            .await?;
        serde_json::from_value(res).map_err(|e| e.to_string())
    }

    async fn drop_database(&self, params: &ConnectionParams, database: &str) -> Result<(), String> {
        let res = self
            .process
            .call(
                "drop_database",
    ...
    async fn rename_database(
        ...
        let res = self
            ...
            .call(
                "rename_database",
                json!({ "params": params, "database": database, "new_name": new_name }),
            )
            .await?;
        serde_json::from_value(res).map_err(|e| e.to_string())
    }

    async fn create_schema(&self, params: &ConnectionParams, schema: &str) -> Result<(), String> {
        let res = self
            .process
            .call(
                "create_schema",
