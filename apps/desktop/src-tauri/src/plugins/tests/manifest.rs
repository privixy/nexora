...
fn records_manifest_fixture_deserialization_and_defaults() {
    let fixtures = [
        (include_str!("../../../../../../plugins/fixtures/manifests/minimal-driver.json"), true),
        (include_str!("../../../../../../plugins/fixtures/manifests/full-driver.json"), true),
        (include_str!("../../../../../../plugins/fixtures/manifests/ui-only.json"), false),
        (include_str!("../../../../../../plugins/fixtures/manifests/aliases.json"), true),
        (include_str!("../../../../../../plugins/fixtures/manifests/unknown-capability.json"), true),
        (include_str!("../../../../../../plugins/fixtures/manifests/unknown-slot.json"), false),
    ];
    for (fixture, accepted) in fixtures {
        assert_eq!(serde_json::from_str::<ConfigManifest>(fixture).is_ok(), accepted);
    }
...
fn records_alias_and_unknown_field_behavior() {
    let aliases = parse(include_str!("../../../../../../plugins/fixtures/manifests/aliases.json"));
    assert!(!aliases.capabilities.connection_string);
    assert_eq!(aliases.capabilities.connection_string_example, "alias://example");
    ...
    let unknown = parse(include_str!("../../../../../../plugins/fixtures/manifests/unknown-capability.json"));
    assert_eq!(unknown.id, "unknown-capability");
    assert!(serde_json::from_str::<ConfigManifest>(include_str!("../../../../../../plugins/fixtures/manifests/unknown-slot.json")).is_err());
}
