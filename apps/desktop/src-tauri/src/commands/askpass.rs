...
pub fn start_frontend_server() -> Result<crate::askpass::AskpassServer, String> {
    let app = APP_HANDLE
        .get()
        .ok_or_else(|| "Askpass UI unavailable: application not initialised".to_string())?;
    crate::askpass::AskpassServer::start(Arc::new(FrontendUi { app: app.clone() }))
}
...
impl crate::askpass::AskpassUi for FrontendUi {
    fn request(&self, kind: crate::askpass::PromptKind, prompt: &str) -> Option<String> {
    ...
    fn show_notification(&self, prompt: &str) -> u64 {
        ...
        let payload = AskpassRequestPayload {
            id,
            kind: crate::askpass::PromptKind::Notify.as_str(),
            prompt: prompt.to_string(),
        };
        if let Err(e) = self.app.emit(REQUEST_EVENT, payload) {
            eprintln!("[Askpass] Failed to notify frontend: {}", e);
