...
pub(crate) const APPROVAL_POLL_INTERVAL_MS: u64 = 500;
...
pub(crate) async fn run_mcp_server() {
    eprintln!("[MCP] Starting Nexora MCP Server...");

    crate::drivers::bootstrap::register_all_drivers(|| {
        let app_config = config::load_config_from_disk();
        (
            app_config.plugins.unwrap_or_default(),
            app_config.active_external_drivers,
        )
    })
    .await;
...
pub(crate) async fn handle_request(req: JsonRpcRequest) -> Option<JsonRpcResponse> {
    // Notifications (no id)
    if req.id.is_none() {
...
async fn handle_list_resources() -> Result<Value, JsonRpcError> {
    ...
    resources.push(Resource {
        uri: "nexora://connections".to_string(),
    ...
    for conn in connections {
        resources.push(Resource {
            uri: format!("nexora://{}/schema", conn.id),
...
async fn handle_read_resource(params: Option<Value>) -> Result<Value, JsonRpcError> {
    ...
    if uri.starts_with("nexora://") && uri.ends_with("/schema") {
        let parts: Vec<&str> = uri.split('/').collect();
        // uri format: nexora://{id}/schema -> ["nexora:", "", "{id}", "schema"]
        if parts.len() < 4 {
            return Err(JsonRpcError {
                code: -32602,
...
async fn tool_run_query(
    ...
    let max_rows = args.get("limit").and_then(|v| v.as_u64()).unwrap_or(100) as u32;
    ...
    if config::is_connection_readonly(config, &conn.id) && kind != "select" {
        ...
        let msg = "Query blocked by Nexora read-only mode. Enable writes for this connection in Settings → MCP → Read-only mode.".to_string();
    ...
    if needs_approval {
        ...
        if !heartbeat::is_alive() {
            ...
            let msg = "Nexora app is not running — open it to approve writes".to_string();
            audit.error = Some(msg.clone());
            return Err(JsonRpcError {
                code: -32000,
                message: msg,
        ...
        .await
        {
            Ok(PollOutcome::Decided(decision)) => {
                if decision.decision == "approve" {
                        ...
                        .filter(|s| !s.is_empty())
                    {
                        ...
                        if config::is_connection_readonly(config, &conn.id) && new_kind != "select"
                        {
                            audit.status = "blocked_readonly".to_string();
                            let msg = "Edited query blocked by Nexora read-only mode. Enable writes for this connection in Settings → MCP → Read-only mode.".to_string();
                            audit.error = Some(msg.clone());
                            return Err(JsonRpcError {
                                code: -32000,
                                message: msg,
            ...
            Ok(PollOutcome::TimedOut) => {
                ...
                let msg = format!(
                    "Approval timed out after {}s — open Nexora to approve writes",
                    timeout_secs
                );
                audit.error = Some(msg.clone());
                return Err(JsonRpcError {
            ...
            Ok(PollOutcome::HostUnavailable) => {
                audit.status = "host_unavailable".to_string();
                let msg =
                    "Nexora app closed during approval — open it to approve writes".to_string();
                audit.error = Some(msg.clone());
                return Err(JsonRpcError {
                    code: -32000,
                    message: msg,
