...
/// Path to a notebook file: `<root>/<connectionId>/<id>.nexora-notebook`.
fn notebook_path(root: &Path, connection_id: &str, notebook_id: &str) -> Result<PathBuf, String> {
    ...
    Ok(dir.join(format!("{}.nexora-notebook", notebook_id)))
}

/// Legacy flat path (pre per-connection layout): `<root>/<id>.nexora-notebook`.
fn legacy_notebook_path(root: &Path, notebook_id: &str) -> Result<PathBuf, String> {
    validate_id(notebook_id)?;
    Ok(root.join(format!("{}.nexora-notebook", notebook_id)))
}
...
fn load_in(root: &Path, connection_id: &str, notebook_id: &str) -> Result<Option<String>, String> {
    ...
    if legacy.exists() {
        ...
        if fs::rename(&legacy, &path).is_err() {
            ...
            fs::write(&path, &content).map_err(|e| format!("Failed to migrate notebook: {}", e))?;
...
fn list_in(root: &Path, connection_id: &str) -> Result<Vec<NotebookMetadata>, String> {
    ...
    for entry in entries.flatten() {
        ...
        let id = match path.file_name().and_then(|n| n.to_str()) {
            Some(name) if name.ends_with(".nexora-notebook") => {
                name.trim_end_matches(".nexora-notebook").to_string()
            }
            _ => continue,
        };
